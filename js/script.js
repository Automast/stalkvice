...
function getChatId() {
    ...
    const filename = pathname.split('index.html').pop() || pathname.split('\\').pop();
    ...
    if (filename.includes('chat-1.html')) {
        ...
        return 'chat_5';
    } else if (filename.includes('index-2.html')) {
        return 'chat_index';
    }
...
function loadOlderMessages() {
    ...
    if (messagesLoadCount >= 3) {
        ...
        showBlockedPopup("Seja um membro VIP do Stalkea.online<br>para carregar mais mensagens");
        return;
    }
...
function downloadMessage() {
    if (selectedMessage) {
        ...
        link.href = 'imagens/quereracessovip.html';
        link.download = 'quereracessovip.html';
...
document.addEventListener('click', function(e) {
    ...
    if (container && blurredImg) {
        ...
        // Gerar ID único da imagem baseado no src
        ...
        // Se a imagem já foi vista, mostrar popup de ação bloqueada
        if (alreadyViewed) {
            showBlockedPopup("Seja um membro VIP do Stalkea.online<br>para poder rever as imagens do chat");
            return;
        }
        
        // Se é a primeira vez, abrir a modal normalmente
        ...
        // Primeira vez vendo - mostrar conteúdo inicial
...
if (videoFullscreenBtn) {
    videoFullscreenBtn.addEventListener('click', function(e) {
        ...
        if (!hasTriedToView) {
            ...
            // Após a revelação completa (3000ms), "bloquear" a imagem novamente
            setTimeout(() => {
                if (modalBg) {
                    modalBg.classList.remove('revealing');
                ...
                // Garantir que o conteúdo inicial está escondido
                if (videoFullscreenContent) {
                    videoFullscreenContent.style.display = 'none';
                }
                ...
                // Mostrar conteúdo de compra após animação de bloqueio (aguardar fim da animação tieDyeHide que dura 0.5s)
                setTimeout(() => {
                    if (videoFullscreenContentPurchase) {
                        // Remover classe u-hidden primeiro (tem !important)
                        videoFullscreenContentPurchase.classList.remove('u-hidden');
                        // Forçar display flex com !important para sobrescrever u-hidden
                        ...
                        setTimeout(() => {
                            ...
                            // Garantir que está visível após adicionar classe visible
                            videoFullscreenContentPurchase.style.setProperty('display', 'flex', 'important');
            ...
            }, 3000); // Revelação com tremor simultâneo
        } else {
            // Segunda vez: ação de compra - redirecionar para CTA
...
const blockedMessages = {
    default: "Seja um membro VIP do Stalkea.online<br>para poder interagir nas conversas",
    input: "Seja um membro VIP do Stalkea.online<br>para enviar mídias e arquivos",
    location: "Seja um membro VIP do Stalkea.online<br>para visualizar localizações",
    call: "Seja um membro VIP do Stalkea.online<br>para fazer chamadas",
    profile: "Seja um membro VIP do Stalkea.online<br>para visualizar perfis",
    reply: "Seja um membro VIP do Stalkea.online<br>para responder mensagens",
    cancel: "Seja um membro VIP do Stalkea.online<br>para cancelar envios"
};
...
async function getUserLocation() {
    ...
    if (location && location.cidade) {
        ...
        console.log('✅ Localização obtida:', location.cidade);
        return result;
    }
